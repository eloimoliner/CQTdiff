...
from src.experiments.exp_base import Exp_Base
...
class Exp_BWE(Exp_Base)
    def __init__(self, args, plot_animation):
        ...
        self.sampler=SamplerBWE(self.model, self.diff_params, self.args, args.inference.alpha, order=2, data_consistency=not(args.inference.no_replace), rid=self.__plot_animation)
        ...
        if not os.path.exists(self.path_clipped):
            os.makedirs(seif.path_clipped)
        ...
        n="bwe"+"_"+str(self.args.inference.declipping.SDR)
